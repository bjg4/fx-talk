...
2. Connect the mic’s curly cable to a compatible USB audio input adapter connected to this Mac. The currently verified adapter is Cable Creation (C-Media). A new adapter must provide an actual audio input at 48 kHz and pass the 16.5–19.5 kHz button signals. An output-only headphone dongle does not suffice.
